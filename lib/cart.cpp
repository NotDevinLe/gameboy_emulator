...
static const std::unordered_map<uint8_t, const char*> LIC_CODE = {
    {0x00, "None"},
    {0x01, "Nintendo R&D1"},
    {0x08, "Capcom"},
...
bool cart_load(const char *cart) {
    ...
    ctx.filename = cart;
    ctx.rom_size = static_cast<uint32_t>(size);
    ctx.rom_data.resize(static_cast<size_t>(size));
    file.read(reinterpret_cast<char*>(ctx.rom_data.data()), size);
    ctx.header = reinterpret_cast<const rom_header*>(ctx.rom_data.data() + 0x0100);
