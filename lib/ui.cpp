...
// SDL2 includes - adjust path as needed for your system
#ifdef __APPLE__
    #include <SDL2/SDL.h>
...
void ui_init() {
    // Prevent SDL from overriding SIGINT/SIGTERM so Ctrl+C works
...
void display_tile(SDL_Surface *surface, uint16_t startLocation, uint16_t tileNum, int x, int y) {
    ...
    for (int tileY = 0; tileY < 16; tileY += 2) {
        ...
        for (int bit = 7; bit >= 0; bit--) {
            uint8_t hi = ((b1 & (1 << bit)) ? 1 : 0) << 1;
            uint8_t lo = (b2 & (1 << bit)) ? 1 : 0;
...
void update_dbg_window() {
    ...
    SDL_UpdateTexture(sdlDebugTexture, nullptr, debugScreen->pixels, debugScreen->pitch);
    SDL_RenderClear(sdlDebugRenderer);
    SDL_RenderCopy(sdlDebugRenderer, sdlDebugTexture, nullptr, nullptr);
